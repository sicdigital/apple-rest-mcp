...
// Safe mode: when true, loadModule logs on-demand loads.
let safeModeFallback = false;

export function setSafeMode(v: boolean): void {
	safeModeFallback = v;
}

// Module cache — populated eagerly (by index.ts) or lazily (on first use).
...
/**
 * Build a fully-configured MCP Server (tool list + call handler) without
 * connecting any transport. Reused by both the stdio and HTTP entry points.
 */
...
async function handleToolCall(
	...
	try {
		...
		// Read-only scope enforcement: block outbound messaging operations.
